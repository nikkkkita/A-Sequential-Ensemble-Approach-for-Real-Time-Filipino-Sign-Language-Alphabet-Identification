...
def main():
    ...
    model_path_model_1 = 'FINALMODEL_knn_rf_classifier.joblib'
    knn_rf_model = joblib.load(model_path_model_1)
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                print("CNN:", keypoint_classifier_labels[hand_sign_id])
                print("Probability of CNN:", hand_sign_prob)

                import numpy as np
                import string

                # KNN + RF Model Prediction
                # You might need to preprocess `pre_processed_landmark_list` if needed by your KNN+RF model
                final_prediction = knn_rf_model.predict([pre_processed_landmark_list])
                # Map numeric prediction to alphabetical labels (0 -> A, 1 -> B, ..., 25 -> Z)
                label_map = {i: letter for i, letter in enumerate(string.ascii_uppercase)}
                # Assuming 'final_prediction' contains integers from 0-25
                final_label = label_map.get(final_prediction[0], "Unknown Label")
                ...
                debug_image = draw_info_text(
                    ...
                    final_label  # Pass the final prediction label here
                )
        else:
            point_history.append([0, 0])
...
def draw_info_text(image, brect, handedness, hand_sign_text, finger_gesture_text, final_prediction_label):
    cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[1] - 22), (0, 0, 0), -1)


    info_text = handedness.classification[0].label[0:]
    if hand_sign_text != "Model 0 Label":
        info_text = info_text + ':' + hand_sign_text
    info_text += f' Sequential Label: {final_prediction_label}' 
